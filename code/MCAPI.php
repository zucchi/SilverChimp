...
class MCAPI
{
     ...
     * @return boolean true on success
     */
    public function campaignScheduleBatch($cid, $schedule_time, $num_batches=2, $stagger_mins=5)
    {
        $params = array();
        $params["cid"] = $cid;
        $params["schedule_time"] = $schedule_time;
    ...
    */
     ...
     * @return bool true if the request succeeds, otherwise an error will be thrown
     */
    public function listMergeVarReset($id, $tag)
    {
        $params = array();
        $params["id"] = $id;
        $params["tag"] = $tag;
     ...
     * @return array Array of a the total records match and matching list member data for this page (see Returned Fields for details)
                ...
                array data the data for each member, including:
                    ...
                    string reason_text For unsubscribes only - if the reason is OTHER, the text entered.
     */
    public function listMembers($id, $status='subscribed', $since=null, $start=0, $limit=100, $sort_dir='ASC')
    {
        $params = array();
        $params["id"] = $id;
        $params["status"] = $status;
     ...
     */
    public function getAccountDetails($exclude=array(
))
    {
        $params = array();
     ...
     */
    public function campaignsForEmail($email_address, $options=null)
    {
        $params = array();
        $params["email_address"] = $email_address;
        $params["options"] = $options;
     ...
     */
    public function deviceRegister($mobile_key, $details)
    {
        $params = array();
     ...
     * @return array the method completion status
                string status The status (success) of the call if it completed. Otherwise an error is thrown.
     */
    public function deviceUnregister($mobile_key, $device_id)
    {
        $params = array();
        $params["mobile_key"] = $mobile_key;
     ...
     */
    public function gmonkeyAdd($id, $email_address)
    {
        $params = array();
        $params["id"] = $id;
        $params["email_address"] = $email_address;
     ...
     * @return array an array of API keys including:
                ...
                array data details on the errors which occurred
                    ...
                    string error an error message explaining the error
     */
    public function gmonkeyDel($id, $email_address)
    {
        $params = array();
        $params["id"] = $id;
        $params["email_address"] = $email_address;
